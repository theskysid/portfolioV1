...
// The texture is rasterised with 7% breathing room on each side, because the
// pointer lens can pull samples that far outward before it hits empty texture.
const PAD = 0.07;
...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  const originX = props.align === 'left' ? width * PAD : width / 2;
    ...
    const rasterize = async () => {
      ...
      if (document.fonts?.ready) {
        try {
          ...
          /* fonts API can reject on a failed face; the fallback stack still rasterises */
        }
      }
    ...
    return () => {
      ...
      if (!contextLost) {
        try {
          ...
          /* the context may already be gone on teardown; nothing left to free */
        }
      }
